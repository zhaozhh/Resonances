function [m,m2,ratio,m2mDeltamTau]=findpeaks2(EorO,U,fraction)
%求所有的峰值及最终的谱函数
...
addition=max(U(:,2))*0.2;%增量，m^2的最大值要比是函数的最高值大addition。
fprintf('\n m^2的最大值要比势能的最大值大%.3f   \n ',addition);
fprintf('m的最大值不超过   ：%f \n', sqrt(max(U(:,2))));
fprintf(' \n');
...
first_m2Space =linspace(0,limitm2,num);
firstRatio=spectrum(EorO,U,first_m2Space,num,fraction);
potionOfPeakes=find(diff(sign(diff(firstRatio)))<0)+1;%find out all the maximum.
fprintf('可能的峰的个数为  ：%i\n', length(potionOfPeakes));
disp('大约位置在：');
fprintf('m^2->：%f\n', first_m2Space(potionOfPeakes));
disp('------------------');
...
noumberOfPeakes=numel(potionOfPeakes);
...
if noumberOfPeakes>=1
    ...
    for i=1:noumberOfPeakes
        if i>numel(pointsmin)
            ...
            if potionOfPeakes(i)<pointsmin(i)
        ...
        fprintf('计算第%i个峰:\n', i);
        [m21,ratio1,peak_m2,deltam_i]=onePeak(m2begin,m2end,firstRatio(potionOfPeakes(i)),EorO,U,fraction);
        fprintf('m^2->：%15.10f\n', peak_m2);
        disp('------------------');
        m2=[m2 m21];
        ratio=[ratio ratio1];
...
while (peakValue2<fraction)||(reltol>0.001)%如果峰值点的值太小或者相对误差太大进如下一个循环
